...
### Changed
- Cloud-engine subtitles no longer embed `[speaker N]` tags by default — cues read as on-screen dialogue and match the local engine's output style. Diarization itself still runs and still powers speaker-mapped transcripts. Re-enable the tags per job with the new "Speaker labels in subtitles" toggle in the web UI, or `SPEAKER_LABELS=1` for the CLI.
...
- API requests that omit `engine` now follow the server's `ASR_ENGINE` instead of always assuming Deepgram — on a zero-cloud deploy, a bare `/api/submit` or `/api/estimate` call no longer routes to the cloud engine and fails on the missing key. An invalid `ASR_ENGINE` value falls back to Deepgram (with a logged warning) rather than reaching the UI's engine picker.
- The Cloud engine now reports unavailable when `DEEPGRAM_API_KEY` is not set: the web UI greys it out exactly like the missing local engine, and `/api/submit` rejects keyless cloud jobs with an actionable message instead of letting every file fail in the worker.
- Batch status forwards the local engine's per-file progress (`progress_seconds`/`total_seconds`), and the web UI shows a live percentage while whisper transcribes — long local jobs previously sat on a static "Transcribing..." for hours.
