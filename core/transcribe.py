...
# diarize_model is self-contained: it both enables diarization and selects the model, and
# Deepgram returns HTTP 400 if the legacy `diarize` boolean is also present in the request.
# So when diarizing we send ONLY this (via the SDK addons escape hatch, since deepgram-sdk
# 3.x does not model diarize_model on PrerecordedOptions) and omit `diarize` from the options.
DIARIZE_MODEL = "v2"
...
def resolve_subtitle_language_tag(
    ...
    *,
    ...
    return (
        _SUBTITLE_LANG_MAP.get(candidate) or _SUBTITLE_LANG_MAP.get(base) or NEUTRAL_SUBTITLE_LANG
    )
...
def _resolve_transcripts_folder_path(video_path: Path) -> Path:
    ...
    if (
        "season" in path_str
        or parent_name_lower.startswith("season")
        or parent_name_lower == "specials"
    ):
...
def _get_channel_count(video: Path, stream_index: Optional[int] = None) -> int:
    ...
    try:
        ...
        cmd += ["-show_entries", "stream=channels", "-of", "json", str(video)]
        result = subprocess.run(cmd, capture_output=True, text=True, check=True)
        data = json.loads(result.stdout)
        streams = data.get("streams", [])
        if streams:
...
def extract_audio(video: Path, language: Optional[str] = None) -> Path:
    ...
    try:
        ...
        if tmp.exists():
            tmp.unlink()
        raise RuntimeError(f"FFmpeg audio extraction timed out after 1 hour for: {video.name}")
    return tmp
...
def transcribe_file(
    ...
    # Diarization: send diarize_model=v2 (Deepgram's v2 speaker model) as a query addon.
    # It is mutually exclusive with the legacy `diarize` boolean, which is why `diarize` is
    # deliberately NOT set on `opts` above -- see DIARIZE_MODEL.
...
def write_transcript(resp: dict, dest: Path, speaker_map_path: Optional[Path] = None):
    ...
    if not words:
        logger.warning("No speech detected — skipping transcript generation for: %s", dest.name)
        return
