...
class CLITestRunner:
    ...
    def run_docker_command(
        self, env_vars: Dict[str, str] = None, timeout: int = 300
    ) -> Tuple[bool, str, str]:
        """
    ...
    def verify_file_content(self, filepath: Path, expected_patterns: List[str] = None) -> bool:
    ...
    def cleanup_test_outputs(self):
        ...
        patterns_to_remove = ["*.srt", "*.transcript.speakers.txt", "*.deepgram.json", "*.synced"]
    ...
    def test_setup_check(self) -> bool:
        ...
        self.record_test_result("Setup Check", all_passed, "Environment validation")
    ...
    def test_basic_video_transcription(self) -> bool:
        ...
        if not self.setup_complete:
            self.record_test_result(
                "Basic Video Transcription", False, "Setup not complete", skipped=True
            )
            return False
        ...
        env_vars = {"MEDIA_PATH": str(self.test_data_dir / "videos"), "BATCH_SIZE": "1"}

        success, stdout, stderr = self.run_docker_command(env_vars)

        if not success:
            self.record_test_result("Basic Video Transcription", False, f"Command failed: {stderr}")
            return False

        # Check for SRT file
        srt_files = list(self.test_data_dir.rglob("*.eng.srt"))
        if not srt_files:
            self.record_test_result("Basic Video Transcription", False, "No SRT file created")
            return False
        ...
        self.record_test_result(
            "Basic Video Transcription", valid, "SRT file created and formatted correctly"
        )
        return valid
    ...
    def test_skip_existing_srt(self) -> bool:
        ...
        if not self.setup_complete:
            self.record_test_result("Skip Existing SRT", False, "Setup not complete", skipped=True)
            return False
        ...
        if not srt_files:
            self.record_test_result("Skip Existing SRT", False, "Could not create SRT for test")
            return False
        ...
        if not skipped:
            self.record_test_result("Skip Existing SRT", False, "File was not skipped")
            return False

        self.log("File correctly skipped", "SUCCESS")
        self.record_test_result("Skip Existing SRT", True, "Existing files skipped correctly")
        return True
    ...
    def test_force_regenerate(self) -> bool:
        ...
        if not self.setup_complete:
            self.record_test_result("Force Regenerate", False, "Setup not complete", skipped=True)
            return False
        ...
        if not srt_files:
            self.record_test_result("Force Regenerate", False, "Could not create SRT for test")
            return False
        ...
        if not success:
            self.record_test_result("Force Regenerate", False, "Command failed")
            return False
        ...
        self.record_test_result("Force Regenerate", regenerated, "Existing SRT regenerated")
        return regenerated
    ...
    def test_transcript_generation(self) -> bool:
        ...
        if not self.setup_complete:
            self.record_test_result(
                "Transcript Generation", False, "Setup not complete", skipped=True
            )
            return False
        ...
        if not success:
            self.record_test_result("Transcript Generation", False, "Command failed")
            return False
        ...
        if not transcript_files:
            self.record_test_result("Transcript Generation", False, "No transcript file created")
            return False
        ...
        self.record_test_result(
            "Transcript Generation", valid, "Transcript created with speaker labels"
        )
        return valid
    ...
    def test_transcript_with_speaker_map(self) -> bool:
        ...
        if not speaker_csv.exists():
            ...
            self.record_test_result(
                "Transcript with Speaker Map", False, "Speaker map not provided", skipped=True
            )
            return False
        ...
        if not success:
            self.record_test_result("Transcript with Speaker Map", False, "Command failed")
            return False
        ...
        if not transcript_files:
            self.record_test_result("Transcript with Speaker Map", False, "No transcript created")
            return False
        ...
        self.record_test_result(
            "Transcript with Speaker Map", has_mapped_names, "Character names used in transcript"
        )
    ...
    def test_keyterms_autoload(self) -> bool:
        ...
        if not keyterms_files:
            ...
            self.record_test_result(
                "Keyterms Auto-load", False, "No keyterms file provided", skipped=True
            )
            return False
        ...
        if not success:
            self.record_test_result("Keyterms Auto-load", False, "Command failed")
            return False
        ...
        self.record_test_result(
            "Keyterms Auto-load", keyterms_loaded, "Keyterms detected and loaded"
        )
    ...
    def test_file_list_processing(self) -> bool:
        ...
        if not self.setup_complete:
            self.record_test_result(
                "File List Processing", False, "Setup not complete", skipped=True
            )
            return False
        ...
        if not videos:
            self.record_test_result("File List Processing", False, "No test videos available")
            return False
        ...
        env_vars = {"FILE_LIST_PATH": str(file_list_path)}

        success, stdout, stderr = self.run_docker_command(env_vars)

        if not success:
            self.record_test_result("File List Processing", False, "Command failed")
            return False
        ...
        self.record_test_result(
            "File List Processing", file_processed, "Videos processed from file list"
        )
    ...
    def test_batch_size_limit(self) -> bool:
        ...
        if not self.setup_complete:
            self.record_test_result("Batch Size Limit", False, "Setup not complete", skipped=True)
            return False
        ...
        if len(videos) < 2:
            ...
            self.record_test_result(
                "Batch Size Limit", False, "Insufficient test videos", skipped=True
            )
            return False
        ...
        if not success:
            self.record_test_result("Batch Size Limit", False, "Command failed")
            return False
        ...
        self.record_test_result("Batch Size Limit", limited, "Batch size respected")
        return limited
    ...
    def test_silent_video_handling(self) -> bool:
        ...
        if not silent_video.exists():
            ...
            self.record_test_result(
                "Silent Video Handling", False, "Silent test video not provided", skipped=True
            )
            return False
        ...
        self.record_test_result(
            "Silent Video Handling", handled_gracefully, "Silent video error handled"
        )
    ...
    def test_statistics_generation(self) -> bool:
        ...
        if not self.setup_complete:
            self.record_test_result(
                "Statistics Generation", False, "Setup not complete", skipped=True
            )
            return False
        ...
        if not success:
            self.record_test_result("Statistics Generation", False, "Command failed")
            return False
        ...
        if not new_logs:
            self.record_test_result("Statistics Generation", False, "No new log file created")
            return False
        ...
        try:
            ...
            self.record_test_result(
                "Statistics Generation", has_all_fields, "Stats JSON created with all fields"
            )
            return has_all_fields

        except json.JSONDecodeError as e:
            self.record_test_result("Statistics Generation", False, f"Invalid JSON: {e}")
            return False
